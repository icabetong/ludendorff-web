...
const AssignmentEditor = (props: AssignmentEditorProps) => {
  const assigned = props.assignment?.dateAssigned as Timestamp | null;
  const returned = props.assignment?.dateReturned as Timestamp | null;
  ...
  const [dateAssigned, setDateAssigned] = useState<Date | undefined>(assigned ? new Date(assigned.seconds * 1000) : undefined);
